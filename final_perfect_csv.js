    ...
    if (line.trim() && !line.startsWith('معرف العقد')) {
        ...
        if (parts.length >= 5) {
            ...
            branchIds.forEach(branchId => {
                ...
                // Also map the base branch pattern (without the sequence number)
                const baseBranchPattern = cleanBranchId.replace(/-\d{4}$/, '');
                if (!branchToContractMap.has(baseBranchPattern)) {
                    branchToContractMap.set(baseBranchPattern, contractId);
                }
            });
        }
    }
...
visitLines.slice(1).forEach(line => {
    ...
    // Get the correct contract ID - try exact match first, then base pattern
    let contractId = branchToContractMap.get(branchId);
    if (!contractId) {
        // Try base pattern (remove sequence number)
        const baseBranchPattern = branchId.replace(/-\d{4}$/, '');
        contractId = branchToContractMap.get(baseBranchPattern);
    }
    
    // If still no match, use company-based fallback
    if (!contractId) {
        // For company 0033, use 0033-007 as default
        if (companyId === '0033') {
            contractId = '0033-007';
        } else if (companyId === '0039') {
            contractId = '0039-001';
        } else {
            contractId = `${companyId}-001`;
        }
    }
...
fs.writeFileSync('visits_complete_336_final_perfect.csv', outputContent);

console.log(`Final perfect CSV file created: visits_complete_336_final_perfect.csv`);
