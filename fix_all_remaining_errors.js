...
lines.forEach((line, index) => {
    ...
    // Fix 6: Ensure all required columns are present (expand to 26 columns)
    while (columns.length < 26) {
        columns.push('');
    }

    // Fix 7: Add source data column
    if (!columns[25]) {
...
fs.writeFileSync('visits_complete_336_final_fixed_v2.csv', outputContent);

console.log(`Fixes applied: ${fixesApplied}`);
console.log(`Contract fixes: ${contractFixes}`);
console.log(`Date fixes: ${dateFixes}`);
console.log(`Status fixes: ${statusFixes}`);
console.log(`Output file: visits_complete_336_final_fixed_v2.csv`);
