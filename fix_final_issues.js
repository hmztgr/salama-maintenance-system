...
console.log(`Final fixed CSV file created: visits_complete_336_final_fixed.csv`);
...
// Check for the fixed date
console.log('\nChecking for fixed date (24-Sep-2024):');
const fixedDateRecords = perfectLines.slice(1).filter(line => {
    const cols = line.split(',');
    return cols[5].includes('24-Sep-2024');
});

fixedDateRecords.forEach((line, index) => {
    const cols = line.split(',');
    console.log(`Fixed date record ${index + 1}: ${cols[0]} | ${cols[1]} | ${cols[2]} | ${cols[5]}`);
}); 