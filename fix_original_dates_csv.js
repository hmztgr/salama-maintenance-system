...
contractLines.forEach(line => {
    if (line.trim() && !line.startsWith('معرف العقد')) {
        ...
        if (parts.length >= 6) {
            ...
            // Map company ID to itself (we'll need to map company names later)
            companyNameToIdMap.set(companyId, companyId);
...
const companyNameMapping = {
    ...
    'إدارة اوقاف احمد محمد شمسان (حارة الشام)': '0043',
    'إدارة اوقاف احمد محمد شمسان ( 9 عمائر ومحلات)': '0058',
    'شمسان بلازا': '0048',
...
visitLines.slice(1).forEach(line => {
    ...
    const branchId = locationToBranchMap.get(location);
    if (!branchId) {
        console.log(`Warning: No branch ID found for location "${location}"`);
        return;
    }
...
fs.writeFileSync('visits_complete_336_original_dates.csv', outputContent);

console.log(`Original dates CSV file created: visits_complete_336_original_dates.csv`);
